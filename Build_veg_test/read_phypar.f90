...
!
!svn $Id: read_phypar.F 793 2016-05-10 00:08:25Z arango $
!================================================== Hernan G. Arango ===
     ...
     &                                        'Must between 1 and 4'
            ...
            CASE ('Hout(idWztw)')
              IF (idWztw.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idWztw,:))
            CASE ('Hout(idWqsp)')
              IF (idWqsp.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idWqsp,:))
            CASE ('Hout(idWbeh)')
              IF (idWbeh.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idWbeh,:))
            CASE ('Hout(idU2rs)')
              IF (idU2rs.eq.0) THEN
            ...
            CASE ('Hout(idW3Sd)')
              IF (idW3Sd.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idW3Sd,:))
            CASE ('Hout(idW3St)')
              IF (idW3St.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idW3St,:))
            CASE ('Hout(idWamp)')
              IF (idWamp.eq.0) THEN
            ...
            CASE ('Hout(idUwav)')
              IF (idUwav.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idUwav,:))
            CASE ('Hout(idVwav)')
              IF (idVwav.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idVwav,:))
            CASE ('Hout(idWdif)')
              IF (idWdif.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idWdif,:))
            CASE ('Hout(idWdib)')
              IF (idWdib.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idWdib,:))
            CASE ('Hout(idWdiw)')
              IF (idWdiw.eq.0) THEN
                ...
                RETURN
              END IF
              Npts=load_l(Nval, Cval, Ngrids, Hout(idWdiw,:))
            CASE ('Hout(idTsur)')
            ...
            CASE ('Dout(M2hjvf)')
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2hjvf),ng)=Ldiag(ng)
                Dout(idDv2d(M2hjvf),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2kvrf)')
              IF (M2kvrf.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2kvrf),ng)=Ldiag(ng)
                Dout(idDv2d(M2kvrf),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2bstm)')
              IF (M2bstm.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2bstm),ng)=Ldiag(ng)
                Dout(idDv2d(M2bstm),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2wrol)')
              IF (M2wrol.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2wrol),ng)=Ldiag(ng)
                Dout(idDv2d(M2wrol),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2wbrk)')
              IF (M2wbrk.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2wbrk),ng)=Ldiag(ng)
                Dout(idDv2d(M2wbrk),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2zeta)')
              IF (M2zeta.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2zeta),ng)=Ldiag(ng)
                Dout(idDv2d(M2zeta),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2zetw)')
              IF (M2zetw.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2zetw),ng)=Ldiag(ng)
                Dout(idDv2d(M2zetw),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2zqsp)')
              IF (M2zqsp.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2zqsp),ng)=Ldiag(ng)
                Dout(idDv2d(M2zqsp),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2zbeh)')
              IF (M2zbeh.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2zbeh),ng)=Ldiag(ng)
                Dout(idDv2d(M2zbeh),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2hvis)')
              IF (M2hvis.le.0) THEN
            ...
            CASE ('Dout(M3vjvf)')
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu3d(M3vjvf),ng)=Ldiag(ng)
                Dout(idDv3d(M3vjvf),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M3hjvf)')
              IF (M3hjvf.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu3d(M3hjvf),ng)=Ldiag(ng)
                Dout(idDv3d(M3hjvf),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M3kvrf)')
              IF (M3kvrf.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu3d(M3kvrf),ng)=Ldiag(ng)
                Dout(idDv3d(M3kvrf),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M3bstm)')
              IF (M3bstm.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu3d(M3bstm),ng)=Ldiag(ng)
                Dout(idDv3d(M3bstm),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M3wrol)')
              IF (M3wrol.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu3d(M3wrol),ng)=Ldiag(ng)
                Dout(idDv3d(M3wrol),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M3wbrk)')
              IF (M3wbrk.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu3d(M3wbrk),ng)=Ldiag(ng)
                Dout(idDv3d(M3wbrk),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M3hvis)')
              IF (M3hvis.le.0) THEN
            ...
            CASE ('Dout(M3fveg)')
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu3d(M3fveg),ng)=Ldiag(ng)
                Dout(idDv3d(M3fveg),ng)=Ldiag(ng)
              END DO
            CASE ('Dout(M2fveg)')
              IF (M2fveg.le.0) THEN
              ...
              Npts=load_l(Nval, Cval, Ngrids, Ldiag)
              DO ng=1,Ngrids
                Dout(idDu2d(M2fveg),ng)=Ldiag(ng)
                Dout(idDv2d(M2fveg),ng)=Ldiag(ng)
              END DO
            CASE ('NUSER')
              Npts=load_i(Nval, Rval, 1, Ivalue)
              Nuser=Ivalue(1)
